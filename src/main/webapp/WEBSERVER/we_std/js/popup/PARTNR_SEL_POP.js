...
app.controller("mstCtl", ($scope, $timeout, $filter, uiGridConstants, blockUI) => {
    ...
    $scope.lfn_dataset_init = (name) => {
    	...
    	if (!name || name === "ds_cond") {
        	...
        	//고정 조회 조건(파트너유형) 설정
        	if (Array.isArray($scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_TY)) {
            	$scope.ds_cond.FIX_PARTNR_TY	= $scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_TY;
        	} else if ($scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_TY) {
            	$scope.ds_cond.FIX_PARTNR_TY	= $scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_TY.split(",");
        	}
        	
        	//고정 조회 조건(파트너구분) 설정
        	if (Array.isArray($scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_GB)) {
            	$scope.ds_cond.FIX_PARTNR_GB	= $scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_GB;
        	} else if ($scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_GB) {
            	$scope.ds_cond.FIX_PARTNR_GB	= $scope.ds_conf.POPUP_OPTIONS.FIX_PARTNR_GB.split(",");
        	}
    ...
    $scope.lfn_search = () => {
    	...
    	const param = {queryId: "we.std.popup.PARTNR_SEL_POP.selMstList",	queryType:"selList",	param:$scope.ds_cond};
    ...
    $scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		columnDefs		:
			[
				{displayName:'ID',		field:'PARTNR_ID',		width:'100',	type:'string',	cellClass:'align_cen'},
				{displayName:'거래처명',	field:'PARTNR_NM',		width:'*',		type:'string',	cellClass:'align_lft'},
				{displayName:'CEO',		field:'CEO',			width:'100',	type:'string',	cellClass:'align_cen'},
				...
				{displayName:'구분',		field:'PARTNR_GB_NM',	width:'100',	type:'string',	cellClass:'align_cen'}
			]
	});
...
});//end-app.controller("mstCtl", ($scope, $timeout, $filter, uiGridConstants, blockUI) => {
