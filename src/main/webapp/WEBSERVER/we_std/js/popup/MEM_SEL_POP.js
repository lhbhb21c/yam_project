...
app.controller("mstCtl", ($scope, $timeout, $filter, uiGridConstants, blockUI) => {
    ...
    $scope.lfn_dataset_init = (name) => {
    	...
    	if (!name || name === "ds_cond") {
        	...
        	//고정 조회 조건(멤버유형) 설정
        	if (Array.isArray($scope.ds_conf.POPUP_OPTIONS.FIX_MEM_TY)) {
            	$scope.ds_cond.FIX_MEM_TY	= $scope.ds_conf.POPUP_OPTIONS.FIX_MEM_TY;
        	} else if ($scope.ds_conf.POPUP_OPTIONS.FIX_MEM_TY) {
            	$scope.ds_cond.FIX_MEM_TY	= $scope.ds_conf.POPUP_OPTIONS.FIX_MEM_TY.split(",");
        	}
        	
        	//고정 조회 조건(사용자구분) 설정
        	if (Array.isArray($scope.ds_conf.POPUP_OPTIONS.FIX_USER_GB)) {
            	$scope.ds_cond.FIX_USER_GB	= $scope.ds_conf.POPUP_OPTIONS.FIX_USER_GB;
        	} else if ($scope.ds_conf.POPUP_OPTIONS.FIX_USER_GB) {
            	$scope.ds_cond.FIX_USER_GB	= $scope.ds_conf.POPUP_OPTIONS.FIX_USER_GB.split(",");
        	}
    ...
    $scope.lfn_search = () => {
    	...
    	const param = {queryId: "we.std.popup.MEM_SEL_POP.selMstList",	queryType:"selList",	param:$scope.ds_cond};
    ...
    $scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		columnDefs		:
			[
				...
				{displayName:'구분',			field:'USER_GB_NM',		width:'100',	type:'string',	cellClass:'align_lft'},
				{displayName:'유형',			field:'MEM_TY_NM',		width:'100',	type:'string',	cellClass:'align_cen'},
				{displayName:'소속',			field:'SECTR_NM',		width:'100',	type:'string',	cellClass:'align_cen'},
			]
	});
