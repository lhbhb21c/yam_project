...
app.controller("mstCtl", ($scope, $timeout, $filter, uiGridConstants, blockUI) => {
	...
	$scope.lfn_btn_onClick = (name) => {
		if (name === "CLOSE"){
			top.MDI.closeTab("/we_std/base/CO_SECTR_MNG");
			...
			$scope.mstGrid.data.unshift({ROW_CRUD:"C", DEL_YN:"N", USE_YN:"Y", CO_CD:($scope.ds_code.CO_CD.length > 0 ? $scope.ds_code.CO_CD[0].CODE_CD : null)});
			$timeout(() => {
	...
	$scope.lfn_load_base = () => {
		return new Promise((resolve) => {
			const param = {queryId: "we.std.base.CO_SECTR_MNG.selCoComboList", queryType:"selList", param:{}};
			...
			XUTL.fetch({
				...
				$scope.ds_code.CO_CD_ALL	= response.success;
				$scope.ds_code.CO_CD		= XUTL.findRows(response.success, ["USE_YN", "DEL_YN"], ["Y", "N"]);
	...
	$scope.lfn_search = () => {
		...
		const param = {queryId: "we.std.base.CO_SECTR_MNG.selMstList", queryType:"selList", param:$scope.ds_cond};
	...
	$scope.lfn_run = (name) => {
		...
		const param = 
			{
				queryId	: "we.std.base.CO_SECTR_MNG.save",
				CMD		: name,
	...
	//회사코드 콤보
	const coCombo = NG_CELL.getComboCell("ds_code.CO_CD", {
    	permanent	: true,
    	ngIf		: "lfn_cell_show",
    	ngDisabled	: "lfn_cell_disabled",
    	ngChange	: "lfn_cell_onChange"
	...
	$scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		multiSelect		: true,
		customStateId	: "/we_std/base/CO_SECTR_MNG/mstGrid",
		columnDefs		:
			[
				...
				{displayName:"섹터ID",		field:"SECTR_ID",		width:"100",	type:"string",	cellClass:"align_cen"},
				{displayName:"회사코드",		field:"CO_CD",			width:"100",	type:"string",	cellClass:"align_lft",
					cellTemplate	: coCombo,
					cellFilter		: 'codeName:grid.appScope.ds_code.CO_CD_ALL',
					filter			: NG_GRD.getDispalyValueFilter($scope, "mstGrid"),
					validTypes		: "required"
				},
				{displayName:"섹터명",			field:"SECTR_NM",		width:"*",		type:"string",	cellClass:"align_lft",
