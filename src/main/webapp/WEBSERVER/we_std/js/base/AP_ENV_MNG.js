...
app.controller("mstCtl", ($scope, $timeout, $filter, uiGridConstants, blockUI) => {
	...
	$scope.lfn_btn_onClick = (name) => {
		if (name === "CLOSE"){
			top.MDI.closeTab("/we_std/base/AP_ENV_MNG");
	...
	$scope.lfn_search = (sels) => {
		...
		const param = {queryId: "we.std.base.AP_ENV_MNG.selMstList", queryType:"selList", param:$scope.ds_cond};
		...
		XUTL.fetch({
			...
			if (sels) {
				$timeout(() => {
					NG_GRD.selectRow($scope.mstGrid.gridApi, XUTL.findRows($scope.mstGrid.data, "AP_ENV_CD", XUTL.getValueMatrix(sels, "AP_ENV_CD")[0]));
				});
			}
	...
	$scope.lfn_run = (name) => {
		...
		const param = 
			{
				queryId	: "we.std.base.AP_ENV_MNG.save",
				CMD		: name,
	...
	$scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		columnDefs		:
			[
				...
				{displayName:"ID",			field:"AP_ENV_ID",	width:"100",	type:"string",	cellClass:"align_cen"},
				{displayName:"환경변수코드",		field:"AP_ENV_CD",	width:"150",	type:"string",	cellClass:"align_lft",
					cellTemplate	: inputCell,
					validTypes		: "required"
				},
				{displayName:"환경변수이름",		field:"AP_ENV_NM",	width:"200",	type:"string",	cellClass:"align_lft",
					cellTemplate	: inputCell,
					validTypes		: "required"
				},
				{displayName:"환경변수값",		field:"AP_ENV_VAL",	width:"*",		type:"string",	cellClass:"align_lft",
					cellTemplate	: inputCell,
					validTypes		: "required"
				},
