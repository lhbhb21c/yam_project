...
app.controller("mstCtl", ($scope, $timeout, $filter, $window, uiGridConstants, blockUI) => {
	...
	$scope.lfn_dataset_init = (name) => {
		...
		if (!name || name === "ds_cond") {
			...
			$scope.ds_cond.YM	= moment().format('YYYY-MM');
		}
		if (!name || name === "MST") {
	...
	$scope.lfn_btn_onClick = (name) => {
		if (name === "CLOSE") {
			top.MDI.closeTab("/yam/pc/PWP_YROP");
	...
	$scope.lfn_search = () => {
		...
		const param = {
			querySet	:
				[
				 {queryId: "yam.pc.PWP_YROP.selMstList",	queryType:"selList",	dataName:"mstGrid.data",	param:$scope.ds_cond}
				]
		}
    ...
    $scope.lfn_search_sub = (entity) => {
    	...
    	const param = {
    		querySet	:
    			[
    			 {queryId: "yam.pc.PWP_YROP.selSubList",	queryType:"selList",	dataName:"subGrid.data",	param:entity}
    			]
    	}
	...
	$scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		customStateId	: "/yam/pc/PWP_YROP/mstGrid",
		columnDefs		:
			[
			 	{displayName:'품목코드',	field:'PWP_IN_ITM_CD',		width:'100',	type:'string',	cellClass:'align_cen'},
			 	{displayName:'품명',		field:'PWP_IN_ITM_NM',		width:'*',		type:'string',	cellClass:'align_lft'},
			 	{displayName:'단위',		field:'PWP_IN_ITM_UN_NM',	width:'80',		type:'string',	cellClass:'align_cen'},
			 	{displayName:'지시수량',	field:'PWO_IN_QTY',			width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'투입수량',	field:'PWP_IN_QTY',			width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'수율',		field:'YROP',				width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'투입율',		field:'IN_RT',				width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'생산율',		field:'OUT_RT',				width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"}
			]
	});
    ...
    $scope.subGrid = NG_GRD.instanceGridOptions({
		rowTemplate		: NG_CELL.getRowTemplate({}),
		customStateId	: "/yam/pc/PWP_YROP/subGrid",
		columnDefs		:
			[
				...
				{displayName:'생산번호',	field:'PWP_NO',			width:'100',	type:'string',	cellClass:'align_cen',
			 	...
			 	{displayName:'비율',		field:'PWP_RT',			width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'수율',		field:'YROP',			width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
...
//				}
	...
	$scope.lfn_cell_onClick = (name, entity, grid) => {
		if (grid.options === $scope.mstGrid) {
			if (name.substring(0, 11) ===  "IN_ITM_QTY_") {
				let ym = entity.YYYY + "-" + name.substring(11,13);
				$scope.lfn_search_sub({ITM_CD:entity.IN_ITM_CD, YM:ym});
			}
