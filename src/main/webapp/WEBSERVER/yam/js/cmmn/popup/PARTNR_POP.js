...
app.controller("mstCtl", ($scope, $timeout, $filter, $window, uiGridConstants, blockUI) => {
    ...
    $scope.lfn_search = () => {
    	...
    	const param = {
    		queryId: "yam.cmmn.popup.PARTNR_POP.selMstList",
    		queryType:"selList",
    		param:$scope.ds_cond
    	}
    ...
    $scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		customStateId	: "/yam/cmmn/popup/PARTNR_POPUP/mstGrid",	
		columnDefs		:
			[
			 {displayName:'거래처코드',	field:'PARTNR_CD',		width:'100',	type:'string',	cellClass:'align_cen'},
			 // {displayName:'거래처이름',	field:'PARTNR_NO',		width:'100',	type:'string',	cellClass:'align_cen'},
			 ...
			 {displayName:'파트너구분',	field:'PARTNR_GB_NM',	width:'150',	type:'string',	cellClass:'align_cen'},
			 {displayName:'업태',		field:'BIZTY',			width:'100',	type:'string',	cellClass:'align_cen',	visible:false},
			 {displayName:'업종',		field:'BIZGB',			width:'100',	type:'string',	cellClass:'align_cen',	visible:false}
			]
	});
    NG_GRD.addExcelExportMenu($scope.mstGrid, "파트너목록");
