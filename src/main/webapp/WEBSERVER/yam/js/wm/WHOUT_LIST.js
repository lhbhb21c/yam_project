...
app.controller("mstCtl", ($scope, $timeout, $filter, $window, uiGridConstants, blockUI) => {
	...
	$scope.load = (conf) => {
		...
		//출고정보가 변경 되었으면 다시 조회할 리스너 설정
		XUTL.listenTopMessage("CHANGE_WHOUT", "/yam/wm/WHOUT_LIST", (msg) => {
	...
	$scope.lfn_dataset_init = (name) => {
		...
		if (!name || name === "ds_cond") {
			XUTL.empty($scope.ds_cond, true);
 			XUTL.addRows($scope.ds_cond.WHOUT_GBS, XUTL.getValueMatrix($scope.ds_code.WHOUT_GB, "CODE_CD")[0]);	//출고구분목록
			...
			$scope.ds_cond.DY_FR	= moment().add(-1, 'months').format('YYYY-MM-DD');		//출고시작일 = 현재일 - 1달
			$scope.ds_cond.DY_TO	= moment().format('YYYY-MM-DD');						//출고종료일 = 현재일
		}
		if (!name || name === "MST") {
	...
	$scope.lfn_btn_onClick = (name) => {
		if (name === "CLOSE") {
			top.MDI.closeTab("/yam/wm/WHOUT_LIST");
	...
	$scope.lfn_search = (sels) => {
		...
		const param = {
			querySet	:
				[
				 {queryId: "yam.wm.WHOUT_LIST.selMstList",	queryType:"selList",	dataName:"mstGrid.data",	param:$scope.ds_cond}
				]
		}
		...
		XUTL.fetch({
			...
			if (sels) {
				$timeout(() => {
					NG_GRD.selectRow($scope.mstGrid.gridApi, XUTL.findRows($scope.mstGrid.data, "WHOUT_NO", XUTL.getValueMatrix(sels, "WHOUT_NO")[0]));
				});
			}
    ...
    $scope.lfn_search_sub = (entity) => {
    	...
    	const param = {
    		querySet	:
    			[
    			 {queryId: "yam.wm.WHOUT_LIST.selSubList",	queryType:"selList",	dataName:"subGrid.data",	param:{WHOUT_ID:entity.WHOUT_ID}}
    			]
    	}
	...
	$scope.mstGrid = NG_GRD.instanceGridOptions({
		...
		customStateId	: "/yam/wm/WHOUT_LIST/mstGrid",
		columnDefs		:
			[
				{displayName:'구분',		field:'WHOUT_GB_NM',	width:'120',	type:'string',	cellClass:'align_cen'},
				{displayName:'출고일',		field:'WHOUT_DY',		width:'100',	type:'string',	cellClass:'align_cen'},
				{displayName:'출고번호',	field:'WHOUT_NO',		width:'120',	type:'string',	cellClass:'align_cen',
					cellTemplate: NG_CELL.getAnchorRenderer({ngClick:"lfn_cell_onClick"})
				},
				{displayName:'순번',		field:'WHOUT_SN',		width:'70',		type:'string',	cellClass:'align_cen'},
			 	...
			 	{displayName:'단위',		field:'WHOUT_UN',		width:'80',		type:'string',	cellClass:'align_cen'},
			 	{displayName:'수량',		field:'WHOUT_QTY',		width:'100',	type:'string',	cellClass:'align_rgt',	cellFilter: 'number',	exportType:"number"},
			 	{displayName:'지시번호',	field:'OD_NO',			width:'100',	type:'string',	cellClass:'align_cen',	visible: false},
			 	{displayName:'상태',		field:'WHOUT_ST_NM',	width:'70',		type:'string',	cellClass:'align_cen'}
			]
	});
	NG_GRD.addExcelExportMenu($scope.mstGrid, "출고목록");
    ...
    $scope.subGrid = NG_GRD.instanceGridOptions({
		...
		rowTemplate		: NG_CELL.getRowTemplate({ngClick:"lfn_row_onClick"}),
		customStateId	: "/yam/wm/WHOUT_LIST/subGrid",
		columnDefs		:
			[
			 {displayName:'No.',		field:'WHOUT_SN',		width:'70',		type:'string',	cellClass:'align_cen'},
			 ...
			 {displayName:'수량',			field:'WHOUT_QTY',		width:'150',	type:'string',	cellClass:'align_rgt',	cellFilter:'number',
				 footerCellFilter:'number',
				 footerCellClass:'align_rgt',
	             aggregationType:uiGridConstants.aggregationTypes.sum
			 },
			 {displayName:'단가',			field:'WHOUT_PRC',		width:'150',	type:'string',	cellClass:'align_rgt',	cellFilter:'number',
				 footerCellFilter:'number',
				 footerCellClass:'align_rgt',
	             aggregationType:uiGridConstants.aggregationTypes.sum
			 },
			 {displayName:'금액',			field:'WHOUT_SUM_AM',	width:'150',	type:'string',	cellClass:'align_rgt',	cellFilter:'number',
				 footerCellFilter:'number',
				 footerCellClass:'align_rgt',
	             aggregationType:uiGridConstants.aggregationTypes.sum
			 },
	...
	$scope.lfn_cell_onClick = (name, entity, grid) => {
		if (grid.options === $scope.mstGrid) {
			//출고번호 클릭시 등록화면으로 이동
			if (name === "WHOUT_NO") {
				_YAM.openMdiTab("WHOUT_REG", entity);
			}
		}
